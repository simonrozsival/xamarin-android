...
namespace Xamarin.Android.Net.TEMPORARY
{
	...
	internal sealed partial class NTAuthentication
	{
		...
		internal NTAuthentication(bool isServer, string package, NetworkCredential credential, string? spn, ContextFlagsPalPlaceholder requestedContextFlags, ChannelBinding? channelBinding)
		{
			...
			{
			throw new PlatformNotSupportedException($"Unsupporeted package {package}");
			}

			if (string.IsNullOrWhiteSpace(credential.UserName) || string.IsNullOrWhiteSpace(credential.Password))
			{
			// NTLM authentication is not possible with default credentials which are no-op
			throw new PlatformNotSupportedException("Missing username or password");
			}
		...
		internal unsafe string? GetOutgoingBlob(string? incomingBlob)
		{
			...
			if (_negotiateMessage == null)
			{
				...
				if (_isSpNego) {
					throw new Exception ("SpNego is disabled at the moment");
				}

				decodedOutgoingBlob = _negotiateMessage;
			}
			else
			{
				Debug.Assert(decodedIncomingBlob != null);
				...
				decodedOutgoingBlob = ProcessChallenge(decodedIncomingBlob);
			}
		...
		//         {
		//             throw new Win32Exception(NTE_FAIL, $"error: mech={mech}");
		//         }
